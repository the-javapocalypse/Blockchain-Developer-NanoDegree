...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const diffMins = Math.round(((currentTime - msgTime % 86400000) % 3600000) / 60000); // minute
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            try {
                const blocks = self.chain.filter(_block => _block.hash == hash);
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            try{
                ...
                self.chain.forEach(_block => {
                    // check if block is valid
                    if(!_block.validate()){
                        errorLog.push('Error: Invalid Block # ' + _block.height)
                    }

                    if(prevBlock == null){
                        prevBlock = _block;
                    } else if(_block.previousBlockHash != prevBlock.hash) {
                        errorLog.push('Error: Invalid Previous Block For Block # ' + _block.height);
                    } else {
                        prevBlock = _block;
                    }
                })
